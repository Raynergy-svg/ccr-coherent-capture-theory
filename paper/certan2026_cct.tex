...
Coherence is spatially independent once the radial metallicity gradient is removed (partial Mantel $r = 0.038$, $p = 0.15$; the raw $r = 0.067$ reflects the \feh--$R_{\mathrm{gal}}$ gradient), satisfying a necessary condition for chemical tagging.
...
Coherence decreases weakly toward the inner disc ($\rho = -0.091$, $p = 0.02$; partial $\rho = -0.147$ controlling for \feh).
...
The Mantel test on the 655 GALAH clusters yields a raw $r = 0.067$ ($p = 0.013$). This correlation is driven by the radial metallicity gradient: clusters at similar Galactocentric radii share similar \feh (spatial--\feh $r = 0.113$; chemical--\feh $r = 0.271$).
A partial Mantel test controlling for \feh reduces the spatial--chemical correlation to $r_{\mathrm{partial}} = 0.038$ ($p = 0.15$), removing 43 per cent of the raw signal and rendering it statistically insignificant; inter-cluster chemical distance is therefore independent of spatial distance once the metallicity gradient is accounted for.
...
Clusters in the inner disc ($R < 7.5$~kpc) have a median \co scatter of 0.106~dex, compared with 0.098~dex for outer disc clusters ($R > 8.5$~kpc; Mann--Whitney $p = 7.9 \times 10^{-3}$).

The Kruskal--Wallis test across four radial bins yields $H = 28.8$, $p = 2.4 \times 10^{-6}$.
Partial correlation controlling for \feh yields $\rho_{\mathrm{partial}} = -0.147$, $p = 1.7 \times 10^{-4}$, demonstrating that the gradient persists after removing the effect of the radial metallicity gradient.
...
\caption{Galactic radius coherence gradient. Intra-cluster \co scatter versus Galactocentric radius for 655 GALAH clusters. The running median (solid line) shows a weak decrease in scatter with radius. Outer disc clusters are marginally more chemically coherent than inner disc clusters ($\rho = -0.091$, $p = 0.02$), persisting after metallicity control ($\rho_{\mathrm{partial}} = -0.147$, $p = 1.7 \times 10^{-4}$).}
\label{fig:gradient}
\end{figure}
...
T10 & Mantel (partial $\mid$\,\feh) & 655 & $r = 0.038$, $p = 0.15$ & Spatially independent \\
...
T19 & Galactic gradient    & 655 & $\rho = -0.091$, $p = 0.02$ & Outer disc weakly more coherent \\
...
\item Coherence varies weakly with Galactic environment: outer disc clusters are marginally more coherent than inner disc clusters ($\rho = -0.091$, $p = 0.02$), a gradient that persists after controlling for metallicity ($\rho_{\mathrm{partial}} = -0.147$, $p = 1.7 \times 10^{-4}$).
