...
Coherence is spatially independent once the radial metallicity gradient is removed (partial Mantel $r = 0.018$, $p = 0.46$; the raw $r = 0.071$ reflects the \feh--$R_{\mathrm{gal}}$ gradient), satisfying a necessary condition for chemical tagging.
...
Coherence decreases weakly toward the inner disc ($\rho = -0.108$, $p = 0.008$; partial $\rho = -0.177$ controlling for \feh).
...
Different tests use different subsets depending on which abundance dimensions are required: the full quality-cut sample is ${\sim}535\,000$ stars for tests requiring only \texttt{flag\_sp}~$= 0$, SNR~$> 30$, and a valid \co ratio (e.g.\ field star matching in Section~\ref{sec:matching}), while 214\,104 stars remain when valid abundances in \emph{all} chemical dimensions (\co, \mgfe, \sife, \alfe, \feh) are required simultaneously.

The \co ratio is computed from the individual carbon and oxygen abundances as
\begin{equation}
\mathrm{C/O} = (\mathrm{C/O})_\odot \times 10^{(\mathrm{[C/Fe]} - \mathrm{[O/Fe]})} \,,
\label{eq:co}
\end{equation}
where $(\mathrm{C/O})_\odot = 0.549$ is the solar carbon-to-oxygen ratio \citep{Asplund2009}. This expression recovers the absolute \co number ratio and is insensitive to the overall metallicity scale.
...
These thresholds are set at approximately twice the typical GALAH DR4 internal abundance uncertainty for FGK stars (0.02--0.04~dex for \mgfe and \sife, 0.03--0.05~dex for \cfe and \ofe; \citealt{Buder2024}). We note that a single threshold applied across all stellar types is a simplification; sensitivity tests using thresholds of 0.03 and 0.07~dex yield qualitatively identical results with enrichment factors varying by less than 20 per cent.
...
Uncertainties on $E$ are estimated from the 16th and 84th percentiles of the null distribution: $E_{\mathrm{err}} = (P_{84} - P_{16}) / (2 \times \mathrm{median}(N_{\mathrm{null}}))$. For the aggregate permanence test, this yields $E = 2.97 \pm 0.4$.
...
The distinct chemical compositions are visible in the per-cluster \co distributions, which show tight, well-separated peaks for coherent clusters, in contrast to the broad field star distribution spanning $\mathrm{C/O} = 0.1$--1.5.
...
The Mantel test on the 655 GALAH clusters yields a raw $r = 0.071$ ($p = 0.005$). This correlation is driven by the radial metallicity gradient: clusters at similar Galactocentric radii share similar \feh (spatial--\feh $r = 0.179$; chemical--\feh $r = 0.298$).
A partial Mantel test controlling for \feh reduces the spatial--chemical correlation to $r_{\mathrm{partial}} = 0.018$ ($p = 0.46$), removing 74 per cent of the raw signal and rendering it statistically insignificant; inter-cluster chemical distance is therefore independent of spatial distance once the metallicity gradient is accounted for.
...
The aggregate enrichment under fixed thresholds is $2.97\times$. The higher enrichment under fixed thresholds ($2.97\times$) compared to Mahalanobis matching ($2.03\times$) reflects the different null models: the Mahalanobis null uses cluster-specific covariance ellipsoids that adapt to each cluster's internal scatter, while the fixed-threshold null uses identical matching volumes for all clusters and random centres, providing a more stringent test of whether cluster positions in abundance space are systematically overdense.
...
The data are consistent with permanent chemical identity and rule out decay on timescales shorter than ${\sim}5$~Gyr at $2\sigma$. We note that the oldest age bin (4--10~Gyr) contains only 6 clusters, limiting the constraining power beyond 4~Gyr. The strongest constraint comes from the 0--4~Gyr range, where 224 clusters show no decline in enrichment (Spearman $\rho = +0.07$, $p = 0.30$).

We note that the T14 decay timescale of $\tau = 1.29$~Gyr (Section~\ref{sec:reality}) measures the increase in intra-cluster \co scatter with cluster age --- reflecting the dissolution of the cluster container as field interlopers contaminate the membership list --- whereas the permanence test measures the enrichment of field stars around cluster chemical centroids. These are complementary: the cluster dissolves on ${\sim}1$~Gyr timescales, but the chemistry of its former members does not change.

\subsubsection{Sensitivity to the C/O temperature systematic}

GALAH \co measurements exhibit a temperature-dependent systematic: Spearman $\rho = -0.68$ between $T_{\mathrm{eff}}$ and \co across 360\,000 FGK dwarfs, with a linear slope of $-0.00053$ \co per K. This systematic arises from the temperature sensitivity of the C\,\textsc{i} and O\,\textsc{i} lines used in the abundance determination. The permanence test is conducted using uncorrected \co values, which is conservative: the systematic adds scatter that would blur any real enrichment signal, making the detection of enrichment more difficult rather than easier. We verify that applying a linear $T_{\mathrm{eff}}$ correction (anchored at the solar $T_{\mathrm{eff}}$ of 5778~K) does not qualitatively change the permanence result: the corrected enrichment remains flat across all age bins (Spearman $\rho = +0.08$, $p = 0.22$) and the AIC continues to favour the flat model.
...
The Spearman correlation between $R_{\mathrm{gal}}$ and \co scatter is $\rho = -0.108$ ($p = 0.008$): scatter decreases weakly with increasing Galactocentric radius.
Clusters in the inner disc ($R < 7.5$~kpc) have a median \co scatter of 0.106~dex, compared with 0.096~dex for outer disc clusters ($R > 8.5$~kpc; Mann--Whitney $p = 3.7 \times 10^{-3}$).

The Kruskal--Wallis test across four radial bins yields $H = 28.8$, $p = 2.5 \times 10^{-6}$.
Partial correlation controlling for \feh yields $\rho_{\mathrm{partial}} = -0.177$, $p = 1.1 \times 10^{-5}$, demonstrating that the gradient persists after removing the effect of the radial metallicity gradient.
...
\begin{figure}
\includegraphics[width=\columnwidth]{t19_galactic_radius_distfix_plot.png}
\caption{Galactic radius coherence gradient. Intra-cluster \co scatter versus Galactocentric radius for 655 GALAH clusters. The running median (solid line) shows a weak decrease in scatter with radius. Outer disc clusters are marginally more chemically coherent than inner disc clusters ($\rho = -0.108$, $p = 0.008$), persisting after metallicity control ($\rho_{\mathrm{partial}} = -0.177$, $p = 1.1 \times 10^{-5}$).}
\label{fig:gradient}
\end{figure}
...
T10 & Mantel (partial $\mid$\,\feh) & 655 & $r = 0.018$, $p = 0.46$ & Spatially independent \\
...
T19 & Galactic gradient    & 655 & $\rho = -0.108$, $p = 0.008$ & Outer disc weakly more coherent \\
...
NGC~6253 is a distant (${\sim}1.65$~kpc), intermediate-age (3--5~Gyr), metal-rich ($\mathrm{[Fe/H]} \approx +0.21$) cluster.
Using a 5D chemical template from 31 clean GALAH members (fixed tolerances of \feh $\pm 0.08$, \mgfe $\pm 0.05$, \sife $\pm 0.05$, \co $\pm 0.08$, \alfe $\pm 0.06$), the pipeline yields: 3598 chemical matches $\to$ 1908 parallax-consistent $\to$ 361 also proper-motion-consistent $\to$ 120 also radial-velocity-consistent (within 15~\kms) $\to$ 7 also age-consistent (2--7~Gyr).
This final count matches the ${\sim}7$ false positives expected by chance in the field pool, so no candidate can be confidently identified as a dissolved member.

One chemically consistent match, HD~163560 (Gaia DR3 5953941329394191360, $G = 9.46$), illustrates the kinematic filter: although it lies within the 5D chemical template, its parallax ($1.83$~mas, ${\sim}547$~pc) is incompatible with the cluster distance ($0.61$~mas, $1.65$~kpc), so it is rejected at the parallax stage as a foreground star.

Table~\ref{tab:pipeline} summarizes the pipeline results.

\begin{table}
\centering
\caption{Individual recovery pipeline summary. Each row shows the number of candidates surviving successive filters (chemistry, parallax, proper motion, radial velocity). ``FP'' is the expected number of chance matches at the stage used for the false-positive comparison---the proper-motion stage for NGC~6791, and the full age-resolved funnel ($7$ age-consistent candidates) for NGC~6253. A candidate count comparable to FP indicates the survivors are consistent with chance rather than genuine recovery.}
\label{tab:pipeline}
\begin{tabular}{lcccccc}
\toprule
...
NGC 6253   & 1.65 & 3598    & 1908 & 361 & 120 & 7 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Comparison to known tidal tails}

A natural positive control for the chemical matching pipeline would be to test whether known tidal tail members of nearby clusters (e.g.\ the Hyades and Praesepe tidal tails identified by \citealt{Meingast2019} and \citealt{Jerabkova2021}) are recovered as chemical matches. This test is limited by GALAH's sparse coverage of the nearest clusters (only 3 Hyades members in the survey) but would be feasible with 4MOST's denser sampling of the solar neighbourhood. We note that GALAH's magnitude range ($V \sim 12$--14) preferentially samples stars at 100--2000~pc, largely missing the brightest members of the nearest clusters.
...
The Galactic radius gradient (Section~\ref{sec:gradient}) suggests that the readability of this archive varies weakly with location: the outer disc preserves chemical coherence marginally more effectively, likely due to a quieter dynamical environment.
...
\item Chemical coherence is spatially independent after controlling for the radial metallicity gradient (partial Mantel $r = 0.018$, $p = 0.46$; the raw $r = 0.071$ reflects the \feh--$R_{\mathrm{gal}}$ gradient), satisfying a necessary condition for chemical tagging. Chemically similar clusters are not preferentially spatial neighbours once \feh is removed.
...
\item Coherence varies weakly with Galactic environment: outer disc clusters are marginally more coherent than inner disc clusters ($\rho = -0.108$, $p = 0.008$), a gradient that persists after controlling for metallicity ($\rho_{\mathrm{partial}} = -0.177$, $p = 1.1 \times 10^{-5}$).
...
This work has made use of data from the European Space Agency (ESA) mission {\it Gaia} \citep{GaiaCollaboration2023}, available at \url{https://www.cosmos.esa.int/gaia}, processed by the {\it Gaia} Data Processing and Analysis Consortium (DPAC, \url{https://www.cosmos.esa.int/web/gaia/dpac/consortium}).
