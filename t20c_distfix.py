    ...
    if len(gaia) > 0:
        ...
        for label, mask in [("Plx+PM", combined),
            ...
            if len(final) > 0:
                ...
                if len(merged) > 0:
                    merged.to_csv(f"t20c_candidates_distfix_{label.replace('+','_')}.csv", index=False)
...
# Save
info("\nSaving...")
with open("t20c_results_distfix.txt", "w") as f:
    f.write("\n".join(out))
info("Saved: t20c_results_distfix.txt")
...
plt.tight_layout()
plt.savefig("t20c_ngc6253_distfix_plot.png", dpi=200, bbox_inches="tight")
plt.close()
info("Saved: t20c_ngc6253_distfix_plot.png")
