...
TOL = {}
for d in dims:
    TOL[d] = {"C_O":0.08,"mg_fe":0.05,"si_fe":0.05,"fe_h":0.08,"al_fe":0.06}[d]  # paper fixed tolerances
info(f"\nMatching tolerances:")
for d in dims:
    info(f"  {d}: ±{TOL[d]:.4f}")
    ...
    if len(gaia) > 0:
        ...
        for label, mask in [("Plx+PM", combined),
            ...
            if len(final) > 0:
                ...
                if len(merged) > 0:
                    merged.to_csv(f"t20c_candidates_fixedtol_{label.replace('+','_')}.csv", index=False)
...
# Save
info("\nSaving...")
with open("t20c_results_fixedtol.txt", "w") as f:
    f.write("\n".join(out))
info("Saved: t20c_results_fixedtol.txt")
...
plt.tight_layout()
plt.savefig("t20c_ngc6253_fixedtol_plot.png", dpi=200, bbox_inches="tight")
plt.close()
info("Saved: t20c_ngc6253_fixedtol_plot.png")
